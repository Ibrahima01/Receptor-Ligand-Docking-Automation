...
def dock_molecule(target_smile: str, protein_pdb: str, res_file: str, center_x: float, center_y: float, center_z: float,
    ...
    # Check if the output contains the line with the docking score
    if len(result.stdout.split('\n')) < 30:
        log.warning("Failed to extract docking score from the output:\n{}".format(result.stdout))
        return None

    try:
        docking_score = float(result.stdout.split('\n')[29].split()[1])
    except IndexError:
        log.warning("Failed to extract docking score from the output:\n{}".format(result.stdout))
        return None
